                    ...
                    <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-9 col-md-9 d-flex pr-0 px-md-3">
                                    <div class="form-group mb-0 w-100">
                                        ...
                                        <input type="text"
                                            class="form-control border align-self-end rounded-0 autocomplete-input"
                        ...
                        <form action="{{ route('clinics.search') }}" method="POST">
                            ...
                            <div class="row align-items-end mt-3">
                                <div class="col-5 col-md-6 d-flex pr-0 px-md-3">
                                    <div class="form-group mb-0 w-100">
                                        <label for="">Your Postcode</label>
                                        @if ($postcode)
                                            <input type="text" class="form-control border align-self-end rounded-0"
                                ...
                                <div class="col-4 col-md-3 d-flex pl-0 px-md-3">
                    ...
                    <a href="/clinics/{{ $clinic->id }}">
                        <div class="clinic card px-0 my-3 h-100 rounded-0">
                            <div class="clinic-logo border-bottom d-flex align-items-center">
                                ...
                                @else
                                    <img class="card-img-top img-fluid px-3" src="{{ asset('img/logo.png') }}"
                                        alt="Card image cap">
                                @endif
                            </div>
                            <div class="card-body d-flex flex-wrap align-items-end">
                                <h5 class="card-title align-self-start mb-0">{{ $clinic->name }}</h5>
        ...
        (function($) {
            ...
            $(document).ready(function() {
                ...
                $(".page-link").click(function(e) {
                    ...
                    $(".pagination-text span").text((((currentPage - 1) * 24) + 1) + " - " + (
                        currentPage * 24));
                });
            })
        })(jQuery);
            ...
            @foreach ($clinics->where('approved', 1)->where('visible', 1) as $clinic)
                {
                    ...
                    name: "{{ trim(preg_replace('/\s\s+/', ' ', htmlspecialchars($clinic->name))) }}",
                    link: "{{ $clinic->id }}"
                },
            @endforeach
        ];
            ...
            $(document).ready(function() {
                ...
                $.get('https://api.postcodes.io/postcodes/' + postcode, function(data) {
                    ...
                    @foreach ($clinics as $clinic)
                        @if ($clinic->lat && $clinic->lng)
                            var distance = haversineDistance([latlng.lat, latlng.lng], [
                                {{ $clinic->lat }}, {{ $clinic->lng }}
                            ], true);
                            distance = Math.floor(distance);

                            if (distance <= {{ $within }}) {
                                $(".col-lg-4:contains('{{ $clinic->postcode }}')").attr('data-order',
                                    distance);
                                $(".col-lg-4:contains('{{ $clinic->postcode }}')").show().find(
                                    '.card-footer').append(
                                    "<div class='distance w-100'><small class='text-muted'><i class='fa fa-map-marker'></i> " +
                                    distance + " miles away</small></div>");
                            }
                        @endif
                    @endforeach
                    var changeDivPositions = $(".col-lg-4:visible");
                    changeDivPositions.sort(function(a, b) {
                        return $(a).data("order") - $(b).data("order")

                    });
                    $("#search-results").html(changeDivPositions);
