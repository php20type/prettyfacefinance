...
jQuery(document).ready(function($) {
    ...
    $("#paymentForm-yourCreditCheck").on('submit', function(e) {
        ...
        $.ajax({
            ...
            if(data.status==="HARD_APPROVED") {
                match = 1;
                // card capture url
                $('#card_capture_url').attr('src', data.card_capture_url);
                $('.paymentForm-yourCreditCheck').addClass('paymentForm-hide');
    ...
    $("#paymentForm-cardDetails").on('submit', function(e) {
        ...
        $.ajax({
            ...
            console.log(data);
            console.log(data.application_ref);
            console.log(data.status);
            console.log(data.loan_explanation_doc_url);
            console.log(data.secci_doc_url);
            if(data.statusCode===500) {
                // most likely a field missing?
                $('.paymentForm-cardDetails').addClass('paymentForm-hide');
            ...
            if(data.status==="CARD_SUBMITTED") {
                $('#loan_explanation_doc_url').attr('src', data.loan_explanation_doc_url);
                $('#secci_doc_url').attr('src', data.secci_doc_url);
